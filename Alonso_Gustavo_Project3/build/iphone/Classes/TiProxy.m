...
//Rare exceptions to indicate a bug in the _alonso_gustavo_project3 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_alonso_gustavo_project3 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_alonso_gustavo_project3]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.alonso_gustavo_project3.module"
