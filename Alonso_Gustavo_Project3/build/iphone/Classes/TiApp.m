...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kAlonso_Gustavo_Project3UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kAlonso_Gustavo_Project3UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Alonso_Gustavo_Project3 app using the node.js CLI
